...
%% load data
load([dirs.matTable filesep 'synCaptcha_matTables.mat']);
...
% figSave([dirs.fg filesep 'pioneer_boxplot_scores_binned']);
...
% figSave([dirs.fg filesep 'pioneer_boxplot_scores_binned_XR']);

%% corr responses per tracer (EM,XR)
fg_corrEMXR(EMXRv,tracers,fg,['corr2(EM,XR) per tracer, n = ' num2str(height(EMXRv))]);
% figSave([dirs.fg filesep 'pioneer_correlationEMXR_scores']);
% figSave([dirs.fg filesep 'pioneer_correlationEMXR_scores_nolegend']);

%% Export csv of scores
% writetable(EMXRv,[dirs.output filesep 'EMXRv_pioneer.txt']);