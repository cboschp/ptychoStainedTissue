function fg_tomoSeries_dose_nr_loglog_blackLines(figProps, dataT, ttl, pltBurnt, pltMassLossType, singlePlot, lgnd)
% common vars
dataMarker = 'o';
sz = 50;
szMarker = 5;
...
%     if tsIdxs(i)
        ...
        y = t.resolution_nr;
        loglog(x, y, ...
            '-','color',colors(colorIdx,:),'LineWidth',figProps.lw);
        hold on;
        loglog(x, y, ...
...
% label burned datasets with mass loss type color (red/orange)
if pltMassLossType
    for i = 1:nTomoSeries
        t = dataT(dataT.tomoSeries == alltSeries(i),:);
        colorIdx = strfind(allResins,t.resinID(1));
